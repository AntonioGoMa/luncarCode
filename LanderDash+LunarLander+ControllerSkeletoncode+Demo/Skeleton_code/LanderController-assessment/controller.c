...
{   
    FILE *file = fopen("datalogging2.csv", "w");
    if (file == NULL){
	...
	while(true) {
        ...
        fprintf(file, "x:%f\n%f\n" ,landerstate.x, landerstate.dx);
        fprintf(file, "y:%f\n%f\n",landerstate.y, landerstate.dy);
        fprintf(file, "0:%f\n%f\n", landerstate.O , landerstate.dO );
        fprintf(file, "Thrust:%f\n", landercommand.thrust);
        fprintf(file, "Rotn:%f\n", landercommand.rotn);
...
int main(int argc, char *argv[])
{
    ...
    if ((e = pthread_create(&dash,NULL, dashboard, argv[2])))
        fprintf(stderr, "not created dashbard thread: %s\n", strerror(e));
    
    if ((e = pthread_create(&datalog, NULL, datalogging, NULL)))
        fprintf(stderr, "not created dataloggin thread: %s\n", strerror(e));
